...
function Copy({
  ...
  return (
    <>
      {kicker ? <p className="kicker text-amber animate-fade-up">{kicker}</p> : null}
      <h1
        className={`${kicker ? "mt-6 " : ""}max-w-4xl font-serif text-display font-light text-paper animate-hero-rise`}
      >
        {title}
      </h1>
      <span
        className="mt-8 block h-px w-16 bg-amber animate-fade-up"
        style={{ animationDelay: "160ms" }}
        aria-hidden
      />
      {intro ? (
        <div
          className="mt-6 max-w-2xl text-body text-paper/75 animate-fade-up"
