...
export default function PathFinderResult({
  ...
  async function downloadPdf() {
    // Server-rendered PDF fetched as a file: no print dialog involved, so it
    // behaves identically on desktop and mobile.
    if (pdfDownloading) return;
    setPdfDownloading(true);
    try {
      const params = new URLSearchParams({ track: track.id });
      if (temperament?.code) params.set("t", temperament.code);
      if (pacing) params.set("pacing", pacing);
      const res = await fetch(`/api/download-pdf?${params}`);
      if (!res.ok) throw new Error("pdf failed");
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `cloud-career-path-${track.id}.pdf`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
    } catch {
      // Non-fatal: the result page itself remains the fallback.
    } finally {
      ...
      <div className="rounded-3xl bg-ink p-8 text-paper sm:p-12">
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-paper/30 px-5 py-2.5 text-small font-medium text-paper transition-all duration-300 ease-calm hover:border-amber hover:bg-hover-ink hover:text-amber disabled:opacity-60"
          >
            <svg viewBox="0 0 20 20" className="h-4 w-4 fill-current" aria-hidden>
          ...
          {/* The chip sits on the flat ink card, so hover-ink is the same
              value bg-paper/10 was already resolving to — solid, not a wash.
              The button hover then needs to lift clear of that chip, which is
              what signature does. */}
          <span className="inline-flex items-center gap-1 rounded-lg bg-hover-ink px-3 py-2">
